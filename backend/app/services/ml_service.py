...
class MLService:
    ...
    async def _load_yolo(self):
        ...
        try:
            ...
            # Check for local best.pt in services folder first
            local_model = Path(__file__).parent / "best.pt"
            
            if local_model.exists():
                logger.info(f"Loading custom YOLO model from {local_model}")
                self.yolo_model = YOLO(str(local_model))
            elif settings.yolo_model_path and Path(settings.yolo_model_path).exists():
                logger.info(f"Loading custom YOLO model from {settings.yolo_model_path}")
