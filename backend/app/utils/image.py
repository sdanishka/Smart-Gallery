...
def _convert_exif_value(value: Any) -> Any:
    ...
    Handles IFDRational, tuples, dicts, and nested structures.
    """
    # Handle None
    if value is None:
        return None
    
    # Handle IFDRational and other rational number types
    ...
    if isinstance(value, (tuple, list)):
        converted = [_convert_exif_value(v) for v in value]
        # Filter out None values from binary data
        converted = [v for v in converted if v is not None]
        return converted if converted else None
    
    # Handle dicts recursively (like GPSInfo)
    if isinstance(value, dict):
        converted = {}
        for k, v in value.items():
            conv_v = _convert_exif_value(v)
            if conv_v is not None:
                # Convert key to string for JSON compatibility
                converted[str(k)] = conv_v
        return converted if converted else None
    
    # Handle strings - skip very long values
...
def extract_exif(file_content: bytes) -> Dict[str, Any]:
    ...
    Returns:
        Dictionary of EXIF data (JSON-serializable)
    """
    try:
        ...
        for tag_id, value in exif_data.items():
            ...
            converted_value = _convert_exif_value(value)
            
            if converted_value is not None:
                result[str(tag)] = converted_value
